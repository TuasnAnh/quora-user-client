...
const topicAnswerWrapper = document.getElementById("topic-answer-content");
const topicQuestionWrapper = document.getElementById("topic-question-content");
...
const answerNav = document.getElementById("topic-answer-nav");
const questionNav = document.getElementById("topic-question-nav");
...
window.onload = enableTopicAnswer();
...
function enableTopicAnswer() {
    setTopicNav("answer");
    topicAnswerWrapper.style.display = "block";
    topicQuestionWrapper.style.display = "none";
...
function enableTopicQuestion() {
    ...
    topicQuestionWrapper.style.display = "block";
    topicAnswerWrapper.style.display = "none";
    topicAnswer.innerHTML = "";
...
function getTopicAnswer(topicId) {
    const xhr = new XMLHttpRequest();
    xhr.open("POST", `${API_URL}/user/topic-answer`, true);
...
function getTopicQuestion(topicId) {
    const xhr = new XMLHttpRequest();
    xhr.open("POST", `${API_URL}/user/topic-question`, true);
    xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest"); // Prevent CSRF attacks
    xhr.setRequestHeader('Accept', 'application/json; charset=utf-8');
    xhr.setRequestHeader('Content-type', "application/x-www-form-urlencoded; charset=utf-8");
    xhr.onreadystatechange = function (e) {
...
function addTopicQuestionCard(questions) {
    for (let i = 0; i < questions.length; i++) {
        ...
        topicQuestion.appendChild(htmlToElements(question)[0]);
    }
}

function addTopicAnswerCard(answers) {
    for (let i = 0; i < answers.length; i++) {
        ...
        if (a.content.includes("dots-")) {
            hideList.push(a.answerId);
        }
