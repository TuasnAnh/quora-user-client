...
function getUserInfor() {
    ...
    xhr.open("GET", `${API_URL}/user/user-information?id=` + userId, true);
    ...
    xhr.onreadystatechange = function (e) {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            if (response.credential) {
                document.querySelector(".employment-credential").style.display = "flex";
                document.getElementById("employment-text").innerHTML = "Works at " + response.credential;
            }

            if (response.location) {
                document.querySelector(".location-credential").style.display = "flex";
                document.getElementById("location-text").innerHTML = "Lives in " + response.location;
            }

            if (response.school) {
                document.querySelector(".education-credential").style.display = "flex";
...
function getUserFollowedTopic() {
    const xhr = new XMLHttpRequest();
    xhr.open("GET", `${API_URL}/user/followed-topic?id=` + userId, true);
    xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest"); // Prevent CSRF attacks
    xhr.setRequestHeader('Accept', 'application/json; charset=utf-8');
    xhr.setRequestHeader('Content-type', "application/x-www-form-urlencoded; charset=utf-8");
    xhr.onreadystatechange = function (e) {
